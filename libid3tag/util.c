							  ...
							  id3_length_t *newlength)
{
	...
	*newlength += *newlength / 1000;
	...
	if (compressed) {
		if (compress2(compressed, (uLongf *)newlength, data, (uLong)length, Z_BEST_COMPRESSION) != Z_OK || *newlength >= length) {
			free(compressed);
			compressed = 0;
		} else {
			id3_byte_t *resized;
								...
								id3_length_t newlength)
{
	...
	if (decompressed) {
		...
		if (uncompress(decompressed, (uLongf *)&size, data, (uLong)length) != Z_OK || size != newlength) {
			free(decompressed);
			decompressed = 0;
		}
	}
