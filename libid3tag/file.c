...
signed long query_tag(FILE *iofile)
{
	...
	read = fread(query, 1, sizeof(query), iofile);
	size = id3_tag_query(query, (id3_length_t)read);
...
struct id3_file *new_file(FILE *iofile, enum id3_file_mode mode,
	...
	file->iofile  = iofile;
	file->mode    = mode;
	file->path    = path ? /*strdup(path)*/ StrDup(path) : 0;
 ...
 */
static
int v1_write(struct id3_file *file, id3_byte_t const *data, id3_length_t length)
{
	...
	if (data) {
		...
		if (fseek(file->iofile, (file->flags & ID3_FILE_FLAG_ID3V1) ? -128 : 0, SEEK_END) == -1 ||
...
extern void GetFileName(char *buf);
/*
 * NAME:	v2_write()
			 ...
			 id3_byte_t const *data, id3_length_t length)
{
	...
	if (data &&
		...
		strcat(newFilename, ".tmp.mp3\0");
		fHandle = fopen(newFilename, "wb");
		if (fHandle) {
			fseek(file->iofile, 0, SEEK_END);			// end of file
			size = ftell(file->iofile);
			if (file->ntags == 0 || (file->ntags == 1 && (file->flags & ID3_FILE_FLAG_ID3V1))) {
				// has no tag or only an id3v1 so copy the whole file
				fseek(file->iofile, 0, SEEK_SET);
			} else {
				fseek(file->iofile, file->tags[0].location + (int)file->tags[0].length, SEEK_SET);
				size -= ftell(file->iofile);			// length of file minus id3v2 tag
			}

			fwrite(data, 1, length, fHandle);			// write tag
			CopyFileData(fHandle, file->iofile, size);  // copy rest of file
			fflush(fHandle);
			fclose(fHandle);

			fclose(file->iofile);
			errChk(DeleteFile(filename));
			errChk(RenameFile(newFilename, filename));
...
/*****************************************************************/
/*** Copies the mp3 file. We can't do it in one fwrite because ***/
/*** allocating the size of entire file is SLOW and attempting ***/
/*** to declare the full size could cause a stack overflow.	   ***/
/*****************************************************************/
#define BLOCK_SIZE 		32768	// about .1 sec faster than 8192

size_t CopyFileData(FILE *dest, FILE *src, int bytesToCopy)
{
	size_t	bytesWritten=0;
	char	buf[BLOCK_SIZE];

	while (bytesWritten < bytesToCopy) {
		fread(buf, 1, bytesToCopy-bytesWritten >= BLOCK_SIZE ? BLOCK_SIZE : bytesToCopy-bytesWritten, src);
		bytesWritten +=
			fwrite(buf, 1, bytesToCopy-bytesWritten >= BLOCK_SIZE ? BLOCK_SIZE : bytesToCopy-bytesWritten, dest);
	}
...
int id3_file_update(struct id3_file *file)
{
	...
	if (v2_write(file, id3v2, v2size) == -1)
		goto fail;
	if (v1_write(file, id3v1, v1size) == -1)
		goto fail;
