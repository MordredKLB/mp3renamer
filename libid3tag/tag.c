...
struct id3_tag *id3_tag_new(void) {
	...
	if (tag) {
		...
		tag->extendedflags = 0;
		tag->restrictions  = 0;
		// setting ID3_TAG_OPTION_UNSYNCHRONISATION causes problems in Winamp when there is an embedded image file
		tag->options       = 0;
		tag->options       = ID3_TAG_OPTION_UNSYNCHRONISATION |
							 /*ID3_TAG_OPTION_COMPRESSION |*/ ID3_TAG_OPTION_CRC;	 // using compression caused some problems when text strings were compressed, so disabling this
		tag->nframes       = 0;
		tag->frames        = 0;
		tag->paddedsize    = 0;
	}
 ...
 * 				if no id specified, return the nth frame.
 */
...
id3_length_t id3_tag_render(struct id3_tag const *tag, id3_byte_t *buffer)
{
	...
	if (flags & ID3_TAG_FLAG_EXTENDEDHEADER) {
		...
		if (ehsize_ptr)
			id3_render_syncsafe(&ehsize_ptr, (unsigned long)ehsize, 4);
	...
	if (!(flags & ID3_TAG_FLAG_FOOTERPRESENT) &&
			!(tag->options & ID3_TAG_OPTION_NOPADDING)) {
	...
	if (tagsize_ptr)
		id3_render_syncsafe(&tagsize_ptr, (unsigned long)(size - 10), 4);

	if (crc_ptr) {
		id3_render_syncsafe(&crc_ptr, id3_crc_compute(frames_ptr, *ptr - frames_ptr), 5);
	}
